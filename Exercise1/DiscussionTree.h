...
public:
	...
	bool printFromResponse(const string& content) const;				// prints the response's subtree (2.9)
...
private:
	...
	void _printNode(const Node& node, ostream& os = cout, size_t indentations = 0) const; // prints a node and it's comments
	Node* root;
};
